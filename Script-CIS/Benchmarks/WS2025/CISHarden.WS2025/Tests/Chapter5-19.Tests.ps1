    ...
    InModuleScope 'CISHarden.WS2025' {
        ...
        Context 'Print Spooler DC/MS (5.1 vs 5.2)' {
            It '5.1 (DC only) NotApplicable en un Member Server' {
                ...
                (Test-CIS_WS2025_5_1).Status | Should -Be 'NotApplicable'
            }
    ...
    InModuleScope 'CISHarden.WS2025' {
        ...
        Context 'DWord simple sobre HKCU (19.5.1.1 Toast notifications on lock screen)' {
            It 'Pass cuando el valor es 1' {
                Mock Get-ItemProperty { [pscustomobject]@{ NoToastApplicationNotificationOnLockScreen = 1 } } -ModuleName CISHarden.Core
                (Test-CIS_WS2025_19_5_1_1).Status | Should -Be 'Pass'
            }
            It 'Fail cuando la clave no existe (default)' {
                Mock Get-ItemProperty { $null } -ModuleName CISHarden.Core
                (Test-CIS_WS2025_19_5_1_1).Status | Should -Be 'Fail'
            }
        }
        ...
        Context 'Mark of the Web clasico (19.7.5.1, distinto del control nuevo 18.10.29.2)' {
            It 'Pass cuando SaveZoneInformation = 2 (se preserva la zona)' {
                Mock Get-ItemProperty { [pscustomobject]@{ SaveZoneInformation = 2 } } -ModuleName CISHarden.Core
                (Test-CIS_WS2025_19_7_5_1).Status | Should -Be 'Pass'
            }
            It 'Fail cuando SaveZoneInformation = 1 (no se preserva)' {
                Mock Get-ItemProperty { [pscustomobject]@{ SaveZoneInformation = 1 } } -ModuleName CISHarden.Core
                (Test-CIS_WS2025_19_7_5_1).Status | Should -Be 'Fail'
            }
        }
        ...
        Context 'Backup de registro con prefijo HKCU (regresion del bug encontrado antes de implementar este capitulo)' {
            It 'Set-CISRegistryValue no rompe con una ruta HKCU y llama a reg export con la ruta traducida' {
                Mock Get-CISServerRole { 'MS' }
                Mock Get-CISServerRole { 'MS' } -ModuleName CISHarden.Core
                Mock Test-Path { $true } -ModuleName CISHarden.Core
                Mock New-Item {} -ModuleName CISHarden.Core
                Mock reg {} -ModuleName CISHarden.Core
                Mock New-ItemProperty {} -ModuleName CISHarden.Core
...
Describe 'Cobertura total del benchmark (454 controles)' {
    InModuleScope 'CISHarden.WS2025' {
        It 'Existe una funcion Test-CIS_*/Set-CIS_* para cada una de las 454 filas del inventario' {
            ...
            foreach ($row in $inventory) {
                $suffix = $row.control_id -replace '\.', '_'
                if (-not (Get-Command "Test-CIS_WS2025_$suffix" -ErrorAction SilentlyContinue)) { $missing += "Test-CIS_WS2025_$suffix" }
                if (-not (Get-Command "Set-CIS_WS2025_$suffix" -ErrorAction SilentlyContinue)) { $missing += "Set-CIS_WS2025_$suffix" }
            }
