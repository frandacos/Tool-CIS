...
<#
    ...
    Todo corre dentro de InModuleScope 'CISHarden' (funciones privadas).
#>
    ...
    InModuleScope 'CISHarden' {
        ...
        Context 'Firewall state (9.1.1 Domain)' {
            It 'Pass cuando EnableFirewall = 1' {
                Mock Get-ItemProperty { [pscustomobject]@{ EnableFirewall = 1 } }
                (Test-CIS_9_1_1).Status | Should -Be 'Pass'
            }
            It 'Fail cuando la clave no existe (firewall no forzado via GPO)' {
                Mock Get-ItemProperty { $null }
                (Test-CIS_9_1_1).Status | Should -Be 'Fail'
            }
        }

        Context 'Logging size limit (9.2.5 Private)' {
            It 'Pass con 16384 o mas' {
                Mock Get-ItemProperty { [pscustomobject]@{ LogFileSize = 32768 } }
                (Test-CIS_9_2_5).Status | Should -Be 'Pass'
            }
            It 'Fail con menos de 16384 (default de Windows es 4096)' {
                Mock Get-ItemProperty { [pscustomobject]@{ LogFileSize = 4096 } }
                (Test-CIS_9_2_5).Status | Should -Be 'Fail'
            }
        }
        ...
        Context 'Logging Name configurado (9.3.6 Public)' {
            It 'Fail cuando la clave no existe' {
                Mock Get-ItemProperty { $null }
                (Test-CIS_9_3_6).Status | Should -Be 'Fail'
            }
            It 'Pass cuando hay una ruta configurada' {
                Mock Get-ItemProperty { [pscustomobject]@{ LogFilePath = '%SystemRoot%\System32\logfiles\firewall\publicfw.log' } }
                (Test-CIS_9_3_6).Status | Should -Be 'Pass'
            }
        }

        Context 'Controles unicos de Public (9.3.4/9.3.5 Apply local rules)' {
            It '9.3.4 Pass cuando AllowLocalPolicyMerge = 0' {
                Mock Get-ItemProperty { [pscustomobject]@{ AllowLocalPolicyMerge = 0 } }
                (Test-CIS_9_3_4).Status | Should -Be 'Pass'
            }
            It '9.3.5 Fail cuando AllowLocalIPsecPolicyMerge no esta en 0' {
                Mock Get-ItemProperty { [pscustomobject]@{ AllowLocalIPsecPolicyMerge = 1 } }
                (Test-CIS_9_3_5).Status | Should -Be 'Fail'
            }
        }
