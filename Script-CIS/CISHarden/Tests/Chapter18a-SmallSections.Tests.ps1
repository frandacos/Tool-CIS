    ...
    InModuleScope 'CISHarden' {
        ...
        Context 'DWord simple (18.1.1.1 Prevent enabling lock screen camera)' {
            It 'Pass cuando NoLockScreenCamera = 1' {
                Mock Get-ItemProperty { [pscustomobject]@{ NoLockScreenCamera = 1 } }
                (Test-CIS_18_1_1_1).Status | Should -Be 'Pass'
            }
            It 'Fail cuando la clave no existe (default)' {
                Mock Get-ItemProperty { $null }
                (Test-CIS_18_1_1_1).Status | Should -Be 'Fail'
            }
        }
        ...
        Context 'Valor contraintuitivo (18.4.1 UAC restrictions, MS only)' {
            It 'Pass cuando LocalAccountTokenFilterPolicy = 0 (restriccion activa)' {
                ...
                Mock Get-ItemProperty { [pscustomobject]@{ LocalAccountTokenFilterPolicy = 0 } }
                (Test-CIS_18_4_1).Status | Should -Be 'Pass'
            }
            It 'Fail cuando vale 1 (sin restriccion)' {
                Mock Get-CISServerRole { 'MS' }
                Mock Get-ItemProperty { [pscustomobject]@{ LocalAccountTokenFilterPolicy = 1 } }
                (Test-CIS_18_4_1).Status | Should -Be 'Fail'
            }
        }

        Context 'Valor REG_SZ (18.5.1 AutoAdminLogon)' {
            It "Pass cuando AutoAdminLogon = '0'" {
                Mock Get-ItemProperty { [pscustomobject]@{ AutoAdminLogon = '0' } }
                (Test-CIS_18_5_1).Status | Should -Be 'Pass'
            }
            It "Fail cuando AutoAdminLogon = '1' (logon automatico habilitado)" {
                Mock Get-ItemProperty { [pscustomobject]@{ AutoAdminLogon = '1' } }
                (Test-CIS_18_5_1).Status | Should -Be 'Fail'
            }
        }

        Context 'Rango con limite superior (18.5.11 WarningLevel)' {
            It 'Pass con 90' {
                Mock Get-ItemProperty { [pscustomobject]@{ WarningLevel = 90 } }
                (Test-CIS_18_5_11).Status | Should -Be 'Pass'
            }
            It 'Fail con 95 (mas del 90% permitido)' {
                Mock Get-ItemProperty { [pscustomobject]@{ WarningLevel = 95 } }
                (Test-CIS_18_5_11).Status | Should -Be 'Fail'
            }
        }

        Context 'Baja confianza declarada (18.11.2)' {
            It 'Es ManualReviewRequired (no se invento la clave de registro)' {
                (Test-CIS_18_11_2).Status | Should -Be 'ManualReviewRequired'
            }
        }
