    ...
    InModuleScope 'CISHarden' {
        ...
        Context 'Event Log con Retention en REG_SZ (18.10.26.2.1 Security)' {
            It "Pass cuando Retention = '0'" {
                Mock Get-ItemProperty { [pscustomobject]@{ Retention = '0' } }
                (Test-CIS_18_10_26_2_1).Status | Should -Be 'Pass'
            }
            It "Fail cuando Retention no es '0'" {
                Mock Get-ItemProperty { [pscustomobject]@{ Retention = '30' } }
                (Test-CIS_18_10_26_2_1).Status | Should -Be 'Fail'
            }
        }

        Context 'Event Log MaxSize (18.10.26.2.2 Security, 196608 KB minimo)' {
            It 'Pass con 196608' {
                Mock Get-ItemProperty { [pscustomobject]@{ MaxSize = 196608 } }
                (Test-CIS_18_10_26_2_2).Status | Should -Be 'Pass'
            }
            It 'Fail con el default de Windows (20480 KB aprox)' {
                Mock Get-ItemProperty { [pscustomobject]@{ MaxSize = 20480 } }
                (Test-CIS_18_10_26_2_2).Status | Should -Be 'Fail'
            }
        }

        Context 'RDP / Terminal Services (18.10.57.3.9.5 Encryption level High)' {
            It 'Pass con MinEncryptionLevel = 3' {
                Mock Get-ItemProperty { [pscustomobject]@{ MinEncryptionLevel = 3 } }
                (Test-CIS_18_10_57_3_9_5).Status | Should -Be 'Pass'
            }
            It 'Fail con un nivel menor' {
                Mock Get-ItemProperty { [pscustomobject]@{ MinEncryptionLevel = 1 } }
                (Test-CIS_18_10_57_3_9_5).Status | Should -Be 'Fail'
            }
        }
        ...
        Context 'WinRM Client vs Service (mismo nombre de valor, distinta ruta)' {
            It '18.10.90.1.1 (Client) y 18.10.90.2.1 (Service) evaluan independientemente' {
                ...
                (Test-CIS_18_10_90_1_1).Status | Should -Be 'Pass'
                (Test-CIS_18_10_90_2_1).Status | Should -Be 'Fail'
            }
        }

        Context 'Windows Defender MAPS (18.10.42.5.2 Join MAPS Advanced)' {
            It 'Pass con SpynetReporting = 2' {
                Mock Get-ItemProperty { [pscustomobject]@{ SpynetReporting = 2 } }
                (Test-CIS_18_10_42_5_2).Status | Should -Be 'Pass'
            }
        }
        ...
        Context 'Controles declarados Manual por baja confianza' {
            It 'Los 11 controles inciertos devuelven ManualReviewRequired' {
                ...
                foreach ($id in $manualIds) {
                    (& "Test-CIS_$id").Status | Should -Be 'ManualReviewRequired' -Because "Test-CIS_$id deberia ser Manual"
                }
            }
        }
