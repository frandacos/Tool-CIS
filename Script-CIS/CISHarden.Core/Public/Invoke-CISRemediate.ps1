function Invoke-CISRemediate {
    <#
    .SYNOPSIS
        Corre Set-CIS_<Tag>_* del benchmark indicado solo para los
        controles que la auditoria marco 'Fail', con 4 niveles de alcance:
        un control puntual, una lista de controles ("grupo"), una
        subseccion, un capitulo completo, o el benchmark entero. Nunca toca
        controles en 'Pass', 'NotApplicable' ni 'ManualReviewRequired'.
        Generico: no sabe nada de ningun benchmark en particular.

    .PARAMETER Benchmark
        Tag del benchmark a remediar (ej. 'WS2025'). Usa Get-CISBenchmarks
        para ver los instalados.
    ...
    .PARAMETER Level
        ...
        'Next Generation Windows Security'. Sirve como alcance propio o
        combinado con -Chapter/-Section.

    .PARAMETER All
        Remediar todos los controles del benchmark de una. Hay que pasar
        este switch explicitamente -- es la unica forma de disparar una
        remediacion masiva; sin alcance la funcion tira error en vez de
        asumir "todo".

    .PARAMETER Force
        Salta la confirmacion de lote ("vas a remediar N controles,
        seguro?"). -WhatIf sigue funcionando igual con o sin -Force.

    .PARAMETER LogPath
        Exporta el detalle de la remediacion a un CSV, ademas de mostrarlo
        en pantalla.
    ...
    .EXAMPLE
        Invoke-CISRemediate -Benchmark WS2025 -All -Force -LogPath .\remediacion_full.csv
    #>
    ...
    if (-not $ControlId -and -not $ControlIds -and -not $Section -and -not $Chapter -and -not $Level -and -not $All) {
        throw "Especifica un alcance: -ControlId, -ControlIds, -Section, -Chapter, -Level, o -All (para remediar todo el benchmark). " +
              "Sin alcance no se ejecuta nada -- es a proposito, para que nunca se dispare una remediacion masiva sin querer."
    }

    $functionPrefix = (Get-CISBenchmarkInfo -Benchmark $Benchmark).FunctionPrefix

    $auditParams = @{ Benchmark = $Benchmark }
    ...
    $scopeDescription =
        if ($All) { "todos los controles del benchmark $Benchmark" }
        elseif ($Chapter) { "el capitulo $Chapter de $Benchmark" }
        elseif ($Section) { "la seccion $Section de $Benchmark" }
        elseif ($ControlIds) { "$($ControlIds.Count) controles puntuales de $Benchmark ($($ControlIds -join ', '))" }
        elseif ($ControlId) { "el control $ControlId de $Benchmark" }
        else { "los controles $Level de $Benchmark" }
    ...
    foreach ($item in $toFix) {
        ...
        $fn = "Set-CIS_${functionPrefix}_$suffix"
